...
const AccordionItem = ({ faq, isOpen, toggle }) => {
  return (
    <motion.div 
      initial={false}
      className={`mb-4 rounded-[32px] overflow-hidden border transition-all duration-500 ${
        isOpen ? 'bg-white border-[#2563EB] shadow-[0_20px_40px_rgba(37,99,235,0.06)]' : 'bg-slate-50 border-slate-100 hover:border-slate-200'
      }`}
    >
      <button 
        onClick={toggle}
      ...
      >
        <span className={`text-lg font-black italic tracking-tight transition-colors duration-500 ${isOpen ? 'text-[#2563EB]' : 'text-[#111111]'}`}>
          {faq.q}
        </span>
        <div className={`h-10 w-10 rounded-full flex items-center justify-center transition-all duration-500 ${isOpen ? 'bg-[#2563EB] text-white rotate-45' : 'bg-white text-slate-400 group-hover:text-[#111111]'}`}>
...
const FAQ = () => {
  ...
  return (
    <section className="py-32 bg-white relative overflow-hidden" id="faq">
        ...
        <div className="grid lg:grid-cols-12 gap-20">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <p className="text-lg text-slate-500 font-medium leading-relaxed max-w-md">
              We believe in 100% transparency. If your question isn't listed here, our 24/7 support team is just a click away.
            </p>
      ...
      {/* Background Decor */}
      <div className="absolute top-0 right-0 w-1/4 h-1/4 bg-blue-50 -z-10 blur-3xl opacity-50" />
    </section>
  );
};
