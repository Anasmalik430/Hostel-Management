      ...
      <div className="mb-16 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 relative">
        <div className="space-y-4">
          <h2 className="text-7xl font-black italic uppercase tracking-tighter text-[#111111] leading-none">
            DISCOVER <br /><span className="text-[#DC2626]">ELITE</span> STAYS
          </h2>
        </div>
        <div className="lg:absolute lg:right-0 lg:bottom-0 text-right">
          <p className="text-[11px] font-black italic text-[#8EABDC] uppercase tracking-widest mb-1">
            Showing {displayRooms.length} Results
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        {displayRooms.length === 0 && !isLoading && (
          <div className="col-span-full py-32 text-center bg-white rounded-[48px] border border-dashed border-slate-200">
            <p className="text-slate-300 font-black italic uppercase tracking-[0.3em] text-sm">No elite units match your search protocol.</p>
          </div>
        )}
      </div>
    </div>
